...
function   phoneme_sequencing_noCue(subject, practice)
...
vol = 4;
toneVol = 0.08;% 0.15 for external speakers
% playbackdevID = 7; %3; % 3 for external speakers
% capturedevID = 6; %1;%1 external mic

isiTimeBaseSeconds = 0.75; % 0.75 % 0.5 Base Duration of ISI s, NOTE: ISI is actually this + 3.25s (+-0.5s)
...
centeredCircle1 = CenterRectOnPointd(baseCircle, screenXpixels-0.5*baseCircleDiam, 1+0.5*baseCircleDiam); %
centeredCircle2 = CenterRectOnPointd(baseCircle, zeros(size(screenXpixels))+0.5*baseCircleDiam, 1+0.5*baseCircleDiam); %
...
while ~KbCheck
    DrawFormattedText(window, 'Please repeat each speech sound after the stimulus. Press any key to start. ', 'center', 'center', [1 1 1],58);
    Screen('Flip', window);
    WaitSecs(0.001);
end
...
for iB = 1:nBlocks %nBlocks;
    ...
    delTimeBaseSecondsA = 1; % 1 % 0.75 Base Duration of Del s
    delTimeBaseSecondsB = 2.5;
    goTimeBaseSeconds = 0.5; % 0.5 Base Duration Go Cue Duration s
    respTimeSecondsA = 1.25; % 1.25 % 1.5 Response Duration s
    respTimeSecondsB = 3; % for sentences
    %isiTimeBaseSeconds = 0.75; % 0.75 % 0.5 Base Duration of ISI s
    
    cueTimeJitterSeconds = 0.25; % 0.25; % Cue Jitter s
    delTimeJitterSeconds = 0;% 0.5; % Del Jitter s
...
%     % play blank
%     
%     blank = audioread('home/coganlab/Psychtoolbox_Scripts/blank.wav');
...
%     PsychPortAudio('FillBuffer', pahandle, toneVol* blank');
%     PsychPortAudio('Start', pahandle, repetitions, StartCue, WaitForDeviceStart);
%     PsychPortAudio('Volume', pahandle, 0.2); % 0.5
%     blankTimeSecs = ((freqS)+length(blank))./(freqS);
%     blankTimeFrames = ceil(blankTimeSecs / ifi);
...
%         [centeredCircle1; centeredCircle2];
        %Play Sound
        Screen('FillOval', window, [circleColor1; circleColor1]' , [centeredCircle1; centeredCircle2]', [baseCircleDiam]);
